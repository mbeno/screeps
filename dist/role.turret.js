...
function turretDo(turret) {
    var low = _.filter(turret.room.find(FIND_STRUCTURES), (o) => {
        return (o.structureType == STRUCTURE_WALL || o.structureType == STRUCTURE_RAMPART) && o.hits < 300
    });
    if (low.length > 0) {
    ...
    var roads = _.filter(turret.room.find(FIND_STRUCTURES), (structure) => {
        return ((structure.structureType == STRUCTURE_ROAD || structure.structureType == STRUCTURE_CONTAINER) && (structure.hits + 1000) < structure.hitsMax)
    });
    if (roads.length > 0) {
    ...
    var walls = _.filter(turret.room.find(FIND_STRUCTURES), (structure) => {
        return (structure.structureType == STRUCTURE_WALL && structure.hits < c.tower.wall_max_repair) || (structure.structureType == STRUCTURE_RAMPART && structure.hits < 100000)
    });
    if (walls.length > 0) {
